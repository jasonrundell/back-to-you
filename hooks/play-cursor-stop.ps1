# Cursor user hook: plays a task-complete sound when the agent stops, or a
# decision-needed sound when the stop payload indicates the agent ended on a question.
    ...
    process {
        ...
        if ($Value.PSObject -and $Value.PSObject.Properties) {
            foreach ($property in $Value.PSObject.Properties) {
                if ($property.Name -match 'message|response|content|text|question|prompt|mode') {
...
$lastDecisionPath = "$env:USERPROFILE\.cursor\last-decision-sound.txt"
if (Test-Path $lastDecisionPath) {
    $lastDecision = [long](Get-Content $lastDecisionPath -Raw)
    $now = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
    if (($now - $lastDecision) -le 10) {
        exit 0
    }
}

Play-RandomSound "$env:USERPROFILE\.cursor\sounds\task-complete"
